#include "NeuralNetworkRM2Dot3.h"
...
NeuralNetworkRM2Dot3::NeuralNetworkRM2Dot3() {
    m_pcWheels = NULL;
    m_pcRABAct = NULL;
    m_pcProximity = NULL;
    m_pcLight = NULL;
...
void NeuralNetworkRM2Dot3::Init(TConfigurationNode& t_node) {
...
void NeuralNetworkRM2Dot3::ControlStep() {
   ...
   if(m_pcRAB != NULL) {
    ...
    //fill input 24->29 (excluded) for messaging rab inputs
    m_inputs[29] = 1 - (2 / (1 + exp(unNumberMessagingNeighbors2))); // Saturate at 5, and is in [0,1]
    for(int i = 30; i < 34; i++) {
    ...
    CCI_EPuckRangeAndBearingSensor::SReceivedPacket cProcessedRabReadingM3 = m_pcRobotState->GetAttractionVectorToMessagingNeighbors(1.0, 80);   // alpha = 1 (artbitrary value)
    CVector2 cRabReadingM3 = CVector2(cProcessedRabReadingM3.Range, cProcessedRabReadingM3.Bearing);
    len = cRabReadingM3.Length();
    if(len != 0) {
       cRabReadingM3.Normalize(); // now, sRabVectorSum.Length = 1
       cRabReadingM3 *= (2/(1+exp(-len)) - 1);
    }

    UInt8 unNumberMessagingNeighbors3 = m_pcRobotState->GetNumberMessagingNeighbors(80);
    //fill input 24->29 (excluded) for messaging rab inputs
    m_inputs[34] = 1 - (2 / (1 + exp(unNumberMessagingNeighbors3))); // Saturate at 5, and is in [0,1]
    for(int i = 35; i < 39; i++) {
      CRadians cDirection = CRadians::PI*(2*(i-35) + 1)/4;
      Real value = cRabReadingM3.DotProduct(CVector2(1.0, cDirection));
      m_inputs[i] = ((value > 0) ? value : 0); // only 2 inputs (rarely 3) will be dif
    }


    } else {
      for(size_t i = 19; i<39; i++) {
         m_inputs[i] = 0;
      }
    }
   ...
   if ((m_net->outputs[3])->activation < 0.5) {
       m_pcRobotState->SetRangeAndBearingMessageToSend(0 | (m_pcRobotState->GetMessageToSend() & 0x5F ));
   }
   else {
       m_pcRobotState->SetRangeAndBearingMessageToSend(160 | (m_pcRobotState->GetMessageToSend() & 0x5F ));
   }

   if ((m_net->outputs[4])->activation < 0.5) {
       m_pcRobotState->SetRangeAndBearingMessageToSend(0 | (m_pcRobotState->GetMessageToSend() & 0xAF ));
   }
   else {
       m_pcRobotState->SetRangeAndBearingMessageToSend(80 | (m_pcRobotState->GetMessageToSend() & 0xAF ));
   }
...
void NeuralNetworkRM2Dot3::Reset() {
...
void NeuralNetworkRM2Dot3::Destroy() {}

REGISTER_CONTROLLER(NeuralNetworkRM2Dot3, "epuck_nn_controller")
