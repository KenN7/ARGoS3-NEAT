#include "NeuralNetworkRM2Dot05.h"
...
NeuralNetworkRM2Dot05::NeuralNetworkRM2Dot05() {
    m_pcWheels = NULL;
    m_pcRABAct = NULL;
    m_pcProximity = NULL;
    m_pcLight = NULL;
...
void NeuralNetworkRM2Dot05::Init(TConfigurationNode& t_node) {
  CEPuckNEATController::Init(t_node);

  /* Reference model */
  m_pcRobotState = new ReferenceModel2Dot1();
...
void NeuralNetworkRM2Dot05::ControlStep() {
   ...
   if(m_pcRAB != NULL) {
      ...
      cProcessedRabReading = m_pcRobotState->GetAttractionVectorToMessagingNeighbors(1.0, 85);   // alpha = 1 (artbitrary value)
      cRabReading = CVector2(cProcessedRabReading.Range, cProcessedRabReading.Bearing);
      len = cRabReading.Length();
      if(len != 0) {
         cRabReading.Normalize(); // now, sRabVectorSum.Length = 1
         cRabReading *= (2/(1+exp(-len)) - 1);
      }
      ...
      for(int i = 25; i < 29; i++) {
         ...
         Real value = cRabReading.DotProduct(CVector2(1.0, cDirection));
	 //std::cout << "value: " << value << " id: " << i << std::endl;
...
void NeuralNetworkRM2Dot05::Reset() {
...
void NeuralNetworkRM2Dot05::Destroy() {}

REGISTER_CONTROLLER(NeuralNetworkRM2Dot05, "epuck_nn_controller")
