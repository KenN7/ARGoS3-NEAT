...
NeuralNetworkRM1Dot2::~NeuralNetworkRM1Dot2() { delete m_pcRobotState; }
...
void NeuralNetworkRM1Dot2::ControlStep() {
    ...
    if (m_pcProximity != NULL) {
        ...
        CCI_EPuckProximitySensor::SReading cProcessedProxiReading =
            m_pcRobotState->GetProximityReading();
        CVector2 cProxiReading =
            CVector2(cProcessedProxiReading.Value, cProcessedProxiReading.Angle);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            m_inputs[i] =
                (value > 0 ? value : 0); // only 2 inputs (rarely 3) will be different from 0.
        }
    } else {
    ...
    if (m_pcLight != NULL) {
        ...
        CVector2 cLightReading =
            CVector2(cProcessedLightReading.Value, cProcessedLightReading.Angle);
    ...
    if (m_pcRAB != NULL) {
        ...
        CCI_EPuckRangeAndBearingSensor::SReceivedPacket cProcessedRabReading =
            m_pcRobotState->GetAttractionVectorToNeighbors(1.0); // alpha = 1 (artbitrary value)
    ...
    // Apply NN outputs to actuation. The NN outputs are in the range [0,1], we remap this range
    // into [-5:5] linearly.
    m_cNeuralNetworkOutputRange.MapValueIntoRange(m_fLeftSpeed, (m_net->outputs[0])->activation,
                                                  m_cWheelActuationRange);
    m_cNeuralNetworkOutputRange.MapValueIntoRange(m_fRightSpeed, (m_net->outputs[1])->activation,
                                                  m_cWheelActuationRange);
