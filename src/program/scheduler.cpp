...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        unsigned int nNum_runs_per_gen = status.Get_count(MPI::UNSIGNED);

        std::cout << " recv seedn: " << nNum_runs_per_gen << std::endl;
        std::vector<UInt32> vecRandomSeed(nNum_runs_per_gen);
        ...
        try {
            for (auto seed : vecRandomSeed) {
                std::cout << "ID" << id << ": Random Seed received: " << seed << std::endl;
