...
extern double nodetrait_mut_sig; // Amount a mutation_num changes on a link connecting a node that
                                 // changed its trait
extern double weight_mut_power;  // The power of a linkweight mutation
extern double recur_prob; // Prob. that a link mutation which doesn't have to be recurrent will be
                          // made recurrent
...
extern double mutate_add_sensor_prob; // NEW: probability to connect a sensor to all outputs (useful
                                      // if we start with some sensors disconnected).
...
extern unsigned int
    num_runs_post_eval;           // number of times to run the post evaluation of the population.
...
extern bool all_inputs_disconnected; // check if all the inputs are disconnected. It's checked in
                                     // the code when we load a genome from a file.
...
extern inline int randint(int x, int y) { return rand() % (y - x + 1) + x; }

extern inline double randfloat() { return rand() / (double)RAND_MAX; }
...
extern double oldhebbian(double weight, double maxweight, double active_in, double active_out,
                         double hebb_rate, double pre_rate, double post_rate);
...
// NOTE: For an inhibatory connection, it makes sense to
//      emphasize decorrelation on hebbian learning!
extern double hebbian(double weight, double maxweight, double active_in, double active_out,
                      double hebb_rate, double pre_rate, double post_rate);
