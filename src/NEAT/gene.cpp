...
Gene::Gene(const char* argline, std::vector<Trait*>& traits, std::vector<NNode*>& nodes) {
    ...
    ss >> traitnum >> inodenum >> onodenum >> weight >> recur >> innovation_num >> mutation_num >>
        enable;
...
void Gene::print_to_file(std::ostream& outFile) {
    ...
    //	(lnk->out_node)->node_id, lnk->weight, lnk->is_recurrent, innovation_num, mutation_num,
    //enable); outFile.write(strlen(tempbuf),tempbuf);
    outFile << (lnk->in_node)->node_id << " ";
    outFile << (lnk->out_node)->node_id << " ";
    outFile << (lnk->weight) << " ";
    outFile << (lnk->is_recurrent) << " ";
