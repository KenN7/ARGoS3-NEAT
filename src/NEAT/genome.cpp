...
Genome::Genome(int id, std::vector<Trait*> t, std::vector<NNode*> n, std::vector<Link*> links) {
    ...
    for (curlink = links.begin(); curlink != links.end(); ++curlink) {
        ...
        tempgene = new Gene((*curlink)->linktrait, (*curlink)->weight, (*curlink)->in_node,
                            (*curlink)->out_node, (*curlink)->is_recurrent, 1.0, 0.0);
        ...
        for (row = 1; row <= totalnodes; row++) {
            ...
            if ((*cmp == true) && (col > i) && ((col <= maxnode) || (col >= first_output)) &&
                ...
                if (col > row) {
                    ...
                    newgene =
                        new Gene(newtrait, new_weight, in_node, out_node, false, count, new_weight);
                    ...
                    new_weight = randposneg() * randfloat();
                    newgene =
                        new Gene(newtrait, new_weight, in_node, out_node, true, count, new_weight);
...
int Genome::get_last_node_id() { return ((*(nodes.end() - 1))->node_id) + 1; }

double Genome::get_last_gene_innovnum() { return ((*(genes.end() - 1))->innovation_num) + 1; }
...
void Genome::mutate_link_weights(double power, double rate, mutator mut_type) {
    ...
    for (curgene = genes.begin(); curgene != genes.end(); curgene++) {
        ...
        if (!((*curgene)->frozen)) {
            ...
            randnum = randposneg() * randfloat() * power * powermod;
            // std::cout << "RANDOM: " << randnum << " " << randposneg() << " " << randfloat() << "
            // " << power << " " << powermod << std::endl;
...
bool Genome::mutate_add_node(std::vector<Innovation*>& innovs, int& curnode_id, double& curinnov) {
    ...
    if (false) {
        ...
        while (((thegene != genes.end()) && (randfloat() < 0.3)) ||
               ((thegene != genes.end()) && (((*thegene)->lnk->in_node)->gen_node_label == BIAS))) {
            ++thegene;
        }
    ...
    while (!done) {
        ...
        if (theinnov == innovs.end()) {
            ...
            innovs.push_back(new Innovation(in_node->node_id, out_node->node_id, curinnov - 2.0,
                                            curinnov - 1.0, newnode->node_id,
    ...
    // genes.push_back(newgene1);   //Old way to add genes- may result in genes becoming out of
    // order genes.push_back(newgene2);
    ...
    if (found) {
        ...
        while (!done) {
            ...
            if (theinnov == innovs.end()) {
                ...
                // Create the new gene
                newgene = new Gene(((thetrait[traitnum])), newweight, nodep1, nodep2, recurflag,
                                   curinnov, newweight);

                // Add the innovation
                innovs.push_back(new Innovation(nodep1->node_id, nodep2->node_id, curinnov,
                                                newweight, traitnum));
            ...
            // OTHERWISE, match the innovation in the innovs list
                ...
                // Create new gene
                newgene = new Gene(((thetrait[(*theinnov)->new_traitnum])), (*theinnov)->new_weight,
                                   nodep1, nodep2, recurflag, (*theinnov)->innovation_num1, 0);
        ...
        // genes.push_back(newgene);  //Old way - could result in out-of-order innovation numbers in
        // rtNEAT
...
// Immediately connects a randomly chosen sensor which was previously disconnected to all the
// outputs at once. Useful if we start with some (inputs) sensors disconnected. NOTE: need to
// decomment the parameter in NEAT::load_neat_params in order to be taken into account.
        ...
        if (!found) {
            ...
            while (!done) {
                ...
                if (theinnov == innovs.end()) {
                    ...
                    // Create the new gene
                    newgene = new Gene(((thetrait[traitnum])), newweight, sensor, output, false,
                                       curinnov, newweight);

                    // Add the innovation
                    innovs.push_back(new Innovation(sensor->node_id, output->node_id, curinnov,
                                                    newweight, traitnum));
                ...
                // OTHERWISE, match the innovation in the innovs list
                    ...
                    // Create new gene
                    newgene =
                        new Gene(((thetrait[(*theinnov)->new_traitnum])), (*theinnov)->new_weight,
                                 sensor, output, false, (*theinnov)->innovation_num1, 0);
    ...
    while (!((p1gene == genes.end()) && (p2gene == (g->genes).end()))) {
        ...
        */
        ...
        while ((curgene2 != newgenes.end()) &&
               (!((((((*curgene2)->lnk)->in_node)->node_id) ==
                   ((((chosengene)->lnk)->in_node)->node_id)) &&
                  (((((*curgene2)->lnk)->out_node)->node_id) ==
                   ((((chosengene)->lnk)->out_node)->node_id)) &&
        ...
        if (!skip) {
            ...
            if ((((chosengene->lnk)->linktrait)) == 0)
                ...
                traitnum =
                    (((chosengene->lnk)->linktrait)->trait_id) -
                    (*(traits.begin()))->trait_id; // The subtracted number normalizes depending on
            ...
            if (inode->node_id < onode->node_id) {
                ...
                if (curnode == newnodes.end()) {
                    ...
                    if (!(inode->nodetrait))
                        ...
                        nodetraitnum =
                            ((inode->nodetrait)->trait_id) - ((*(traits.begin()))->trait_id);
                ...
                if (curnode == newnodes.end()) {
                    ...
                    if (!(onode->nodetrait))
                        ...
                        nodetraitnum =
                            ((onode->nodetrait)->trait_id) - (*(traits.begin()))->trait_id;
...
Genome* Genome::mate_multipoint_avg(Genome* g, int genomeid, double fitness1, double fitness2,
    ...
    // NEW 3/17/03 Make sure all sensors and outputs are included
    for (curnode = (g->nodes).begin(); curnode != (g->nodes).end(); ++curnode) {
        if ((((*curnode)->gen_node_label) == INPUT) || (((*curnode)->gen_node_label) == OUTPUT) ||
    ...
    while (!((p1gene == genes.end()) && (p2gene == (g->genes).end()))) {
        ...
        if (p1gene == genes.end()) {
            ...
            if (p1innov == p2innov) {
                ...
                ////with alpha=0.5, this will produce babies evenly in exploitation and exploration
                ///space /and uniformly distributed throughout
                // blx_alpha=-0.4;
        ...
        */
...

        {
            ...
            if (((((((*curgene2)->lnk)->in_node)->node_id) ==
                  ((((chosengene)->lnk)->in_node)->node_id)) &&
                 (((((*curgene2)->lnk)->out_node)->node_id) ==
                  ((((chosengene)->lnk)->out_node)->node_id)) &&
                 ((((*curgene2)->lnk)->is_recurrent) == (((chosengene)->lnk)->is_recurrent))) ||
        ...
        if (p1gene == p1stop) {
            ...
            if (p1innov == p2innov) {
                ...
                // We are at the crosspoint here
                    ...
                    // WEIGHTS AVERAGED HERE
                    (avgene->lnk)->weight =
                        (((*p1gene)->lnk)->weight + ((*p2gene)->lnk)->weight) / 2.0;
...
double Genome::compatibility(Genome* g) {
    ...
    while (!((p1gene == genes.end()) && (p2gene == (g->genes).end()))) {
        ...
        if (p1gene == genes.end()) {
            ...
            if (p1innov == p2innov) {
                ...
                // mut_diff+=trait_compare((*p1gene)->lnk->linktrait,(*p2gene)->lnk->linktrait);
                // //CONSIDER TRAIT DIFFERENCES
    ...
    // cout<<"COMPAT: size = "<<max_genome_size<<" disjoint = "<<num_disjoint<<" excess =
    // "<<num_excess<<" diff = "<<mut_diff_total<<"  TOTAL =
    // "<<(disjoint_coeff*(num_disjoint/1.0)+excess_coeff*(num_excess/1.0)+mutdiff_coeff*(mut_diff_total/num_matching))<<std::endl;

    return (NEAT::disjoint_coeff * (num_disjoint / 1.0) + NEAT::excess_coeff * (num_excess / 1.0) +
...
Genome* NEAT::new_Genome_auto(int num_in, int num_out, int num_hidden, int type,
                              const char* filename) {
